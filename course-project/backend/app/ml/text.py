...
def extract_budget_places_range(text: str) -> tuple[Optional[float], Optional[float]]:
    ...
    budget_fragment_pattern = re.compile(
        r"(?:бюджетн\w*\s+мест\w*|мест\w*\s+на\s+бюджет\w*|количеств\w*\s+бюджет\w*\s+мест\w*).*?(?=[.!?]|$)",
        flags=re.IGNORECASE | re.DOTALL,
    )

    fragments = [m.group(0) for m in budget_fragment_pattern.finditer(text_lower)]
    ...
    min_patterns = [
        rf"(?:от|не\s+менее|не\s+меньше|минимум|более|больше|свыше|как\s+минимум)\s+{number_pattern}",
    ]

    max_patterns = [
        rf"(?:до|не\s+более|не\s+больше|максимум|менее|меньше)\s+{number_pattern}",
    ]

    for frag in fragments:
        if places_min is None:
            for pattern in min_patterns:
