...
def _build_explanation(program: Dict[str, Any], query_data: Dict[str, Any], score: float) -> str:
    ...
    if q["detected_budget"] is not None:
        budget_score = _score_budget(q["detected_budget"], p.get("has_budget"))
        if budget_score >= 0.9:
            parts.append("тип обучения по бюджету совпадает")
        else:
            parts.append("тип обучения по бюджету не совпадает")
